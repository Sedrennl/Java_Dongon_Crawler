...
public class Playground {
    ...
    //test et implantation des différents elements du background
    ...
    public ArrayList<Sprite> getSolidSpriteList() {
        ...
        for (Sprite sprite : environment) {
            ...
            if (sprite instanceof TrapSprite) solidSpriteArrayList.add(sprite);
        }
    ...
    public void drawText(Graphics g, String text, int x, int y, Color color, Font font) {
        g.setColor(color); // Définir la couleur du texte
        g.setFont(font); // Définir la police de texte
        g.drawString(text, x, y); // Dessiner le texte aux coordonnées (x, y)
    }
}
